...
def find_logs(bag_dir):
    ...
    k_sorted = [int(f.split('/')[-1].split('_')[-1].replace('.log','')[1:]) for f in files]
    ...
    with open(file) as f:
        for line in f:
            if line[:4]==' * /':
                ...
                value = line.split(':')[1]
                if '[' in value:
                    value = list(map(lambda v: float(v),
                                value
                                .replace('[','')
                                .replace(']','')
                                .split(',')
...
def resample_join(dt, *V):
    ...
    *V: pandas.Series, pandas.DataFrame
    ...
    """
    return pd.concat([d.resample(dt).mean() for d in V],
                     join='inner',
                     axis=1,
                    )

def compute_u(f, df, 
              root_regulation, limit_velocity, approach_velocity,
              decomposed=False
             ):
    ...
    u = ((-2.*s*dy/A).rename('u_dy'),
         (s**2*y/A).rename('u_y'),
         (gamma*(dx1*D+2.*e*x1**2/alpha**2)/(D**2)/A).rename('u_gamma'),
         (-2.*B*x7*abs(x1)*dx1/A).rename('u_B'),
         (x1*(x5+2.*x6*x2)).rename('u_chi')
        )
    #return (-2.*s*dy+pow(s,2)*y+ gamma*(dx1*D+2.*e*pow(x1,2)/pow(alpha,2))/(pow(D,2))-2.*B*x7*abs(x1)*dx1)/A+x1*(x5+2.*x6*x2);
    
    if decomposed:
        return {'y': (x1.rename('x1'),
                      (-gamma*np.arctan(e)).rename('gamma')
                     ),
                'dy': (dx1.rename('dx1'),
                       (-gamma*x1/D).rename('gamma')
                      ),
                'dx1': ((-A*x3).rename('v'),
                        (-A*x4).rename('offset'),
...
class kalman(object):
    ...
    def compute_A(self, x):
        ...
        cA = self.A_coeff
        cB = self.B_coeff
        _A[0,0] = -2.*cB*abs(x[0])*x[5]
        _A[0,1] = cA*(x[3]+2.*x[4]*x[1])
        _A[0,2] = -cA
        _A[0,3] = x[1]*cA
        _A[0,4] = x[1]**2*cA
