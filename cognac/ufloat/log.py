...
class logger:
    ...
    def log(self, **kwargs):
        ...
        # self.df.update(kwargs)

    def finalize(self):
        """conclude logging and convert data into a single dataframe"""
    ...
    def plot(self, **kwargs):
        ...
        ax = self.df.set_index("time").plot(**_dkwargs)
        return ax
    ...
    def plot_bk(self, gridded=True, size=None):
        ...
        if gridded:
            ...
            def bfig(c, x_range):
                s = figure(
                    tools=TOOLS, width=width, height=height, title=c, x_range=x_range
                )
                s.line(_d["time"] / 60, _d[c], legend_label=c, color=col, line_width=3)
                return s

            c0 = next(c for c in _d.columns if c is not "time")
            s = [bfig(c0, None)]
            s = s + [bfig(c, s[0].x_range) for c in _d.columns if c not in ["time", c0]]
            grid = gridplot(s, ncols=2)
            show(grid)
        else:
            s1 = figure(tools=TOOLS, width=width, height=height, title=None)
            iters = ((_d[c], c) for c in _d.columns if c is not "time")
            for c, col in zip(iters, colors):
                s1.line(
                    _d["time"] / 60, c[0], legend_label=c[1], color=col, line_width=3
                )
            show(s1)


#
def plot_logs(logs, f, z_target=None, eta=None, title=None, figsize=(12, 10)):
    ...
    if isinstance(logs, dict) and "state" not in logs:
        ...
        def _plot(ax, log, variable, name, scale=1.0, offset=0.0):
            for key, c in zip(logs, cols):
                t, _log = logs[key][log]["time"], logs[key][log]
                ax.plot(
                    t * sec2min,
                    _log[variable] * scale + offset,
                    color=c,
                    label=name + " " + key,
                )
        ...
        def _plot(ax, log, variable, name, scale=1.0, offset=0.0):
            ...
            ax.plot(
                t * sec2min, _log[variable] * scale + offset, color=cols, label=name
            )
    ...
    if z_target is not None:
        ...
        if eta is not None:
            ax.plot(
                t * sec2min, z_target(t) + eta(t), color="green", label="target+eta"
            )
    ax.legend(loc=0)
    ax.set_ylabel("z [m]")
    ...
    if hasattr(f, "piston"):
        ax.axhline(f.piston.vol_min * 1.0e6, ls="--", color="0.5")
        ax.axhline(f.piston.vol_max * 1.0e6, ls="--", color="0.5")
    ax.legend()
    ax.set_ylabel("[cm^3]")
    ax.grid()
    ax.yaxis.set_label_position("right")
    ...
    if "piston" in log0:
        ...
        print(
            "Extrapolated energy conssumption: %.1e Wh/day = %.1f Wh/30day"
            % (nrg * 86400, nrg * 86400 * 30.0)
        )
...
def plot_kalman(log, f, z_target=None, truth=None, figsize=(10, 6)):
    ...
    for i, name in enumerate(f.kalman.names):
        ...
        if i == 1 and z_target:
            ax.plot(tk, -z_target(log["kalman"]["time"]), color="b", label="target")
        ax.set_title(name)
        ax.grid()
...
def plot_kalman_v0(log, f, V_e=None, gamma_e=None, z_target=None):
    ...
    if z_target is not None:
        ax.plot(t, z_target(log["state"]["time"]), color="b", label="target")
    ax.set_title("z  [m]")
    # ax.set_xlabel("t (min)")
    ax.grid()
    legend = ax.legend(loc="best", shadow=True, fontsize="medium")
    ...
    ax.plot(tk, k["gamma_e"], color="r", label="estimated equivalent compressibility")
    if gamma_e is not None:
        if isinstance(gamma_e, float) or gamma_e.size == 1:
            ax.axhline(gamma_e, color="k")
        else:
            ax.plot(tk, gamma_e, color="k", label="gamma_e")
    else:
        ax.plot(
            tk, k["gammaV"], color="k", label="float compressibility x float volume"
        )
