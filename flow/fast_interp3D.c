/*  Fast C/OpenMP interpolator for tvs_to_s_fast()
    ...
    Where: zt  is a 1D numpy array, size NZ      , type float64
...
static inline void myinterp(double *zsk, double *vsk, double *ztk, double *vtk, npy_uint NZ)
{
...
static PyObject* fast_interp3D(PyObject* self, PyObject* args)
{
    ...
    npy_uint      NX,NY,NZ,nd,i,j,k,typ;
    ...
    // Output array must have the same size as input ndarray vs
    vt = (PyArrayObject*)PyArray_SimpleNew(nd, PyArray_DIMS(vs), NPY_FLOAT64);
...
#pragma omp parallel shared(zt,z,vs,vt,NX,NY,NZ,nd,typ) private(i,j,k,zsk,vsk,ztk,vtk)
    {
      // Thread-local constant and work arrays
      //invH = 1.0/DEPTH(0);
      zsk = malloc(sizeof(double)*NZ);
      vsk = malloc(sizeof(double)*NZ);
      ztk = malloc(sizeof(double)*NZ); for (k=0;k<NZ;k++) ztk[k]=ZT(k);
      vtk = malloc(sizeof(double)*NZ);
#pragma omp for
      for (j=0;j<NY;j++) {
        for (i=0;i<NX;i++) {
          ...
          myinterp(zsk,vsk,ztk,vtk,NZ); // interpolate this column
          for (k=0;k<NZ;k++) VT(k,j,i) = vtk[k]; // copy to output
        } // end for i
      } // end for j
...
PyMODINIT_FUNC PyInit_fast_interp(void)
{
    Py_Initialize();
